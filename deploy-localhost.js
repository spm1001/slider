...
class AppsScriptLocalhostDeployer {
  constructor() {
    this.auth = null;
    this.drive = null;
  ...
  async authenticate() {
    console.log('🔐 Authenticating with Google APIs (Localhost Server)...');
    ...
    try {
      ...
      console.log('🔄 No valid token found, starting localhost authentication flow...');
      await this.getLocalhostToken();
    }
...
Please ensure your credentials.json file is in the project directory.
  ...
  async getLocalhostToken() {
    return new Promise((resolve, reject) => {
            ...
            res.end(`
              <html>
                <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
                  ...
                  <p>You can close this window. The deployment will continue automatically.</p>
      ...
      server.listen(3000, 'localhost', () => {
        console.log('🌐 Started local server on http://localhost:3000');
        ...
        fs.writeFileSync(path.join(this.scriptsFolder, 'localhost-auth-url.txt'), authUrl);
        console.log('💾 URL also saved to: localhost-auth-url.txt');
      });
  ...
  async deployProject() {
    try {
      ...
      console.log('- Make sure port 3000 is not in use by other applications');
      throw error;
    }
  }
}
...
// Main execution
async function main() {
  const deployer = new AppsScriptLocalhostDeployer();
...
module.exports = AppsScriptLocalhostDeployer;