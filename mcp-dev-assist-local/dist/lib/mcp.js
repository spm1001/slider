...
export const createServer = () => {
    const server = new McpServer({
        ...
        version: "1.0.0",
        description: "Google Workspace Docs MCP, provides access to Google Workspace documentation, release notes, and search functionality to find information about Google Workspace APIs and services.",
    ...
    server.resource("Google Workspace Release Notes", "docs://release-notes", async () => {
    ...
    server.tool("read_official_google_documentation_page", "Reads a specific Google developer documentation page. Use this when you need to read a specific documentation page instead of using a web browser. This tool provides special handling for links to Google documentation pages and parsing of documentation content.", {
        link: z
            .string()
            .regex(/^https:\/\/(developers|cloud).google.com\//, "Link must be a URL to a Google documentation page.")
    ...
    server.tool("preview_google_workspace_card", "Generates a preview of a Google Workspace Card framework. Use this tool to visualize how a card will look in Google Workspace applications. card is for Google Workspace Add-ons and Chat Apps. The card is defined using the Google Workspace UI Kit JSON format. Read the docs at `https://developers.google.com/workspace/add-ons/concepts/card-interfaces` for more information on how to create cards.", {
        card: z
            .object({})
            .passthrough()
